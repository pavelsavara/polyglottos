...
{
    public class GRegionGenerator : GContainerGeneratorBase
    {
        protected override void GenerateProlog(IGSnippetContainer snippet)
        {
            var rg = (IGRegion) snippet;
            VerticalSpacingBegin(rg);

            CodeWriter.WriteLine("#region " + rg.Name);
        }
        ...
        {
            var rg = (IGRegion) snippet;
            CodeWriter.WriteLine("#endregion " + rg.Name);
